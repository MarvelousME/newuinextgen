...
Write-Host "Starting clean-install stack ($composeFile) at http://127.0.0.1:8891..."
...
Add-Content -Path $report -Value "`n`n## Latest clean-install run`n- UTC: $stamp`n- WP-CLI exit: $code`n- URL: http://127.0.0.1:8891`n- Compose: $composeFile`n"

if ($code -ne 0) { throw 'Clean-install WP-CLI failed - do not label PRODUCTION PASS.' }

Write-Host ''
Write-Host 'Clean install WP-CLI succeeded.'
Write-Host '  WordPress: http://127.0.0.1:8891'
Write-Host '  Admin:     http://127.0.0.1:8891/wp-admin'
