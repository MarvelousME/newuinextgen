 ...
 */
/**
 * Kinetic homepage â€” amalgamated from nextgen-tutors-kinetic-homepage.html
...
$faqs = [
    ...
    [
        ...
        'a' => sprintf(
            ...
            __( 'Our %s guarantee covers your first lesson â€” we will rematch you with another tutor or refund you. No awkward conversations.', 'beyondinfinity' ),
            bi_guarantee_label()
        ),
    ],
];
...
<section class="ngt-section ngi-home ngi-home--classic">
  <div class="ngt-container">
    <div class="ngt-card" style="padding:48px;text-align:center">
      ...
      <p><?php esc_html_e( 'Personalised tutoring across South Africa â€” online, in-person, and hybrid.', 'beyondinfinity' ); ?></p>
...
<div class="ngi-home" id="nextgen-home">
  <?php /* Floating Book CTA removed â€” right-hand float dock covers global actions. */ ?>
      ...
      <div class="ngi-hero-grid">
        <div>
          ...
          <p class="ngi-lead ngi-reveal">
            <?php echo $cms_hero_lead ? esc_html( $cms_hero_lead ) : esc_html__( 'Connect with background-checked tutors for CAPS, IEB and Cambridge â€” online or in-person, from Grade 1 to varsity.', 'beyondinfinity' ); ?>
          </p>
  ...
  <section class="ngi-section" id="trust">
    <div class="ngi-wrap">
      <div class="ngi-section-head ngi-reveal">
        ...
        <p class="ngi-subtitle"><?php echo esc_html( $cms_trust['subtitle'] ?? __( 'Registration, tutor matching, booking, CRM follow-up, payment status, dashboards and verification â€” built for South African families.', 'beyondinfinity' ) ); ?></p>
      </div>
      <div class="ngi-card-grid">
        <article class="ngi-card ngi-reveal bi-tilt-3d bi-tilt-3d--decorative" data-bi-tilt data-bi-tilt-max="8">
          ...
          <p><?php esc_html_e( 'Select subject, grade, province and format â€” then move into the right registration and placement journey.', 'beyondinfinity' ); ?></p>
          </div>
        </article>
        <article class="ngi-card ngi-reveal bi-tilt-3d bi-tilt-3d--decorative" data-bi-tilt data-bi-tilt-max="8">
          <div class="bi-tilt-3d__inner">
  ...
  <?php if ( bi_home_section_enabled( 'subjects' ) ) : ?>
    <?php if ( function_exists( 'bi_home_3d_use_filmstrip' ) && bi_home_3d_use_filmstrip() ) : ?>
  <section class="ngi-section ngi-alt ngi-home-3d-filmstrip" id="subjects" data-ngt-filmstrip="subjects">
    <div class="ngi-wrap ngi-wrap--filmstrip">
      <?php
			if ( class_exists( 'NGTFS_Renderer' ) && class_exists( 'NGTFS_Data' ) ) {
				echo NGTFS_Renderer::render( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					NGTFS_Data::get_cards( [ 'source' => 'subjects', 'limit' => 10 ] ),
					[
						'title'    => (string) ( $cms_subjects['title'] ?? __( 'Explore subjects in 3D', 'beyondinfinity' ) ),
						'subtitle' => (string) ( $cms_subjects['subtitle'] ?? __( 'Pick a subject and meet matching tutors — drag, swipe, or use arrows.', 'beyondinfinity' ) ),
						'source'   => 'subjects',
						'autoplay' => true,
						'loop'     => true,
						'class'    => 'ngt-home-3d-subjects-film',
					]
				);
			} else {
				echo do_shortcode( '[ngt_filmstrip source="subjects" limit="10" autoplay="yes" loop="yes"]' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			?>
    </div>
  </section>
        ...
        <div class="ngi-subject-tabs ngi-reveal" role="tablist" aria-label="<?php esc_attr_e( 'Subjects', 'beyondinfinity' ); ?>">
          <?php foreach ( $subject_tabs as $i => $tab ) : ?>
            <button class="ngi-tab<?php echo 0 === $i ? ' is-active' : ''; ?>" type="button" role="tab" aria-selected="<?php echo 0 === $i ? 'true' : 'false'; ?>"
              ...
              <?php echo esc_html( $tab['title'] ); ?> <span aria-hidden="true">â†’</span>
            </button>
          <?php endforeach; ?>
        </div>
  ...
  <section class="ngi-section ngi-aura" id="platform-highlights">
    <div class="ngi-wrap">
      <div class="ngi-section-head ngi-reveal">
        ...
        <p class="ngi-subtitle"><?php esc_html_e( 'From vetted tutors to parent dashboards and a first-lesson guarantee â€” every step is built for confidence.', 'beyondinfinity' ); ?></p>
      </div>
      <div class="ngi-feature-grid">
        ...
        <article class="ngi-card ngi-reveal">
          ...
          <p><?php esc_html_e( 'Session notes, goals and report cards stay visible in your dashboard â€” even when motion effects are turned off.', 'beyondinfinity' ); ?></p>
        ...
        <article class="ngi-card ngi-reveal">
          ...
          <p><?php esc_html_e( 'Match a tutor, request an assessment, or apply to teach â€” clear CTAs for parents, students and educators.', 'beyondinfinity' ); ?></p>
        ...
        <article class="ngi-card ngi-reveal">
          ...
          <p><?php esc_html_e( 'Love the first lesson or we rematch â€” or refund. No questions asked.', 'beyondinfinity' ); ?></p>
  ...
  <section class="ngi-section bi-cinematic-band<?php echo $story_video ? ' bi-cinematic-band--has-video' : ''; ?>" id="video-story" aria-labelledby="ngi-story-heading">
    ...
    <div class="ngi-wrap bi-cinematic-band__inner">
      ...
      <div>
        ...
        <article class="ngi-split-item ngi-reveal" style="margin-top:0.85rem"><h3><?php esc_html_e( 'Verified tutor matching', 'beyondinfinity' ); ?></h3><p><?php esc_html_e( 'Subject fit, grade support, province availability and learning format â€” all visible before you book.', 'beyondinfinity' ); ?></p></article>
  ...
  <?php if ( bi_home_section_enabled( 'tutors' ) ) : ?>
    <?php if ( function_exists( 'bi_home_3d_use_filmstrip' ) && bi_home_3d_use_filmstrip() ) : ?>
  <section class="ngi-section ngi-alt ngi-home-3d-filmstrip" id="tutors" data-ngt-filmstrip="tutors">
    <div class="ngi-wrap ngi-wrap--filmstrip">
      <?php
			if ( class_exists( 'NGTFS_Renderer' ) && class_exists( 'NGTFS_Data' ) ) {
				$tutor_cards = NGTFS_Data::get_cards( [ 'source' => 'tutors', 'limit' => 8 ] );
				if ( ! $tutor_cards ) {
					$tutor_cards = NGTFS_Data::get_cards( [ 'source' => 'subjects', 'limit' => 8 ] );
				}
				echo NGTFS_Renderer::render( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					$tutor_cards,
					[
						'title'    => (string) ( $cms_tutors['title'] ?? __( 'Vetted educators ready to book.', 'beyondinfinity' ) ),
						'subtitle' => (string) ( $cms_tutors['subtitle'] ?? __( 'Drag, swipe, or use arrows — every educator is vetted.', 'beyondinfinity' ) ),
						'source'   => 'tutors',
						'autoplay' => true,
						'loop'     => true,
						'class'    => 'ngt-home-3d-tutors-film',
					]
				);
			} else {
				echo do_shortcode( '[ngt_filmstrip source="tutors" limit="8" autoplay="yes" loop="yes"]' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			?>
    </div>
  </section>
  ...
  <section class="ngi-section ngi-alt" id="tutors">
    <div class="ngi-wrap">
      <div class="ngi-section-head ngi-reveal">
        <div class="ngi-eyebrow"><?php echo esc_html( $cms_tutors['eyebrow'] ?? __( 'Featured tutors', 'beyondinfinity' ) ); ?></div>
        <h2 class="ngi-heading" data-bi-slide-title><?php echo esc_html( $cms_tutors['title'] ?? __( 'Vetted educators ready to book.', 'beyondinfinity' ) ); ?></h2>
        <p class="ngi-subtitle"><?php echo esc_html( $cms_tutors['subtitle'] ?? __( 'Verified tutors from our directory â€” CAPS, IEB and Cambridge support.', 'beyondinfinity' ) ); ?></p>
      </div>
        ...
        if ( function_exists( 'bi_render_3d_card_stack' ) && bi_3d_enabled() ) {
            $stack_plans = array_map(
                static function ( $plan ) {
                    return [
                        'title'    => $plan['name'] ?? '',
                        'body'     => ( $plan['price'] ?? '' ) . ' / hr â€” ' . implode( ', ', array_slice( (array) ( $plan['items'] ?? [] ), 0, 2 ) ),
            ...
            bi_render_3d_card_stack(
                $stack_plans,
                [
                    'aria_label' => __( 'Pricing plans â€” hover to fan cards', 'beyondinfinity' ),
  ...
  <section class="ngi-section ngi-section--testimonials" id="reviews">
    <div class="ngi-wrap">
      <div class="ngi-section-head ngi-reveal">
        <div class="ngi-eyebrow"><?php echo esc_html( $cms_reviews['eyebrow'] ?? __( 'Happy clients Â· real marks', 'beyondinfinity' ) ); ?></div>
        ...
        <?php if ( bi_testimonials_use_demo_data() ) : ?>
          <p class="ngi-demo-note" role="note"><?php esc_html_e( 'Demo stories â€” replace with verified reviews from your testimonial directory after launch.', 'beyondinfinity' ); ?></p>
        <?php endif; ?>
      </div>
    ...
    <div class="ngi-wrap">
      <div class="ngi-cta ngi-reveal">
        ...
        <div class="ngi-cta-panel">
          ...
          <p><?php echo esc_html( $cms_cta['guarantee_line'] ?? __( 'Love the lesson â€” or your first hour is on us.', 'beyondinfinity' ) ); ?></p>
