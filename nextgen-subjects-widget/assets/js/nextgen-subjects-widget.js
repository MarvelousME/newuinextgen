(() => {
		...
		bindCardMotion() {
			...
			this.cards.forEach((card) => {
				card.addEventListener('mousemove', (event) => {
					...
					card.style.setProperty('--ng-rotate-x', `${((0.5 - y) * 7).toFixed(2)}deg`);
					card.style.setProperty('--ng-rotate-y', `${((x - 0.5) * 7).toFixed(2)}deg`);
				});
	...
	const initialize = () => {
		document.querySelectorAll('[data-nextgen-subjects]').forEach((root) => {
			if (root.dataset.nextgenInitialized === 'true') {
				return;
			}
